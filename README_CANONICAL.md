...
| `social.js` | Run-cashout share card (end-of-run sharing). Still wired by `study.html` and `daily.html`. |
...
| File | Why it's dead |
|---|---|
| `confidence.js`, `daily.js` | Early drafts of calibration / daily logic written against an **older question schema** (`q.topic`, `q.stem`). The real banks use `q.source` / `q.q`, so these silently no-op. **`aced-core.js` replaces them** with schema-correct versions, and neither is loaded by any live page. (`nemesis.js` was the third draft here but has since been **rebuilt schema-correct and is live** — see the SHIP table above.) |
