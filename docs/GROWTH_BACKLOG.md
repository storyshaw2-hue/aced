...
| 5 | **Spoiler-Free Share Card** | If we generate a Wordle-style result grid (🟩🟨⬜) after the Daily Close with a one-tap copy and an image download, then organic new-user acquisition will increase because low-friction shareables are proven to spread exam-prep tools through existing study communities. | 5 | 3 | 5 | 1.6 | social.js already has share-card scaffolding. Use Canvas API for image export. Include aced.pplx.app URL in card footer. Example grid: `🟩🟨🟩🟩⬜`. |
